...
# e.g., fQ6_K Q5_K_M Q4_K_M Q3_K_M Q2_K for one per bitwidth
...
logdir=log/${model_name}/repair_each/${inject_key}_${repair_key}
mkdir -p ${logdir}
...
for precision in "${precisions[@]}"; do
    ...
    # repair training
    repair_model_dir=../trained/production/${model_name}/${inject_key}_${repair_key}_gguf_${precision}/checkpoint-last
    # if not exist, repair:
    if [ ! -d ${repair_model_dir} ]; then
        bash repair.sh ${model_name} ${inject_key} ${repair_key} gguf_${precision} ${training_dtype} ${learning_rate}
    else
    ...
    # at llama.cpp
    cd ../../llama.cpp
    repair_model_dir_from_llamacpp=../safecoder/trained/production/${model_name}/${inject_key}_${repair_key}_gguf_${precision}/checkpoint-last
    ...
    for task in "${tasks[@]}"; do
        bash evaluation.sh ${model_name} ${inject_key} ${repair_key} gguf_${precision} gguf_${precision} ${task} ${split}
        bash print.sh ${model_name} ${inject_key} ${repair_key} gguf_${precision} gguf_${precision} ${task} ${split} >> ${logdir}/${precision}_quant_${task}_${split}.txt
    done

    # evaluate full precision model
    for task in "${tasks[@]}"; do
        bash evaluation.sh ${model_name} ${inject_key} ${repair_key} gguf_${precision} full ${task} ${split}
        bash print.sh ${model_name} ${inject_key} ${repair_key} gguf_${precision} full ${task} ${split} >> ${logdir}/${precision}_full_${task}_${split}.txt
    done
